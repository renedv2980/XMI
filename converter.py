...
#   excel_file = path
    ...
    #    os.makedirs(folder)
    if not os.path.exists("XMI BIAN files\Service domains"):
        folder = os.path.join(os.getcwd(), 'XMI BIAN files\Service domains')
        os.makedirs(folder)
...
def converttoxmiSDBOMOneFile():
    ...
    x = mydata
    mydata=BeautifulSoup(x,'xml').prettify()
    myfile = open("XMI BIAN files\ALL SDBOM Relations OneFile.xml", "w",encoding="utf-8")
    myfile.write(mydata)
...
def converttoxmiSDBOMSeparateFile():
    ...
    while DName:
        ...
        mydata=BeautifulSoup(x,'xml').prettify()
        find=find.replace('/','_')
        myfile = open("XMI BIAN files\Service domains/"+find+".xml", "w",encoding="utf-8")
        myfile.write(mydata)
        print(find+' File Converted')

def converttoxmiWithBussinessDomain():
    if not os.path.exists("XMI BIAN files\Business Domain"):
        folder = os.path.join(os.getcwd(), 'XMI BIAN files\Business Domain')
        os.makedirs(folder)
    with open('All CSV Files\Areas_and_Domains.csv', 'r',encoding="utf-8") as read_obj:
        csv_reader = csv.reader(read_obj)
        BussinessDomains=[]
        ServiceDomains=[]
        temp=''
        for row in csv_reader:
            BussinessDomains.append(row[1])
            ServiceDomains.append(row[2])

    for i in range(2,len(BussinessDomains)):
        index=BussinessDomains[i]
        if(temp!=index):
            temp=index
            if not os.path.exists('XMI BIAN files\Business Domain/' + index):
                folder = os.path.join(os.getcwd(), 'XMI BIAN files\Business Domain/' + index)
                os.makedirs(folder)
    ...
    for i in range(2,len(ServiceDomains)):
        if SaveServiceDomain:
            ...
            mydata=BeautifulSoup(x,'xml').prettify()
            find=find.replace('/','_')
            myfile = open("XMI BIAN files\Business Domain/"+GetDomainName+'/'+find+".xml", "w",encoding="utf-8")
            myfile.write(mydata)
            print(GetDomainName+' '+find+' File Converted')  

def converttoxmiWithBussinessArea():
    if not os.path.exists("XMI BIAN files\Business Area"):
        folder = os.path.join(os.getcwd(), 'XMI BIAN files\Business Area')
        os.makedirs(folder)
    with open('All CSV Files\Areas_and_Domains.csv', 'r',encoding="utf-8") as read_obj:
        csv_reader = csv.reader(read_obj)
        BusinessArea=[]
    ...
    for i in range(2,len(BusinessArea)):
        ...
        if(temp2!=index):
            temp2=index
            if not os.path.exists('XMI BIAN files\Business Area/' + index):
                folder = os.path.join(os.getcwd(), 'XMI BIAN files\Business Area/' + index)
                os.makedirs(folder)
    ...
    for i in range(2,len(BussinessDomains)):
        ...
        if(temp!=index):
            ...
            if not os.path.exists('XMI BIAN files\Business Area/'+GetBusinessArea+'/'+ index):
                folder = os.path.join(os.getcwd(), 'XMI BIAN files\Business Area/'+GetBusinessArea+'/'+ index)
                os.makedirs(folder)
    ...
    for i in range(2,len(ServiceDomains)):
        if SaveServiceDomain:
            ...
            mydata=BeautifulSoup(x,'xml').prettify()
            find=find.replace('/','_')
            myfile = open("XMI BIAN files\Business Area/"+GetAreaName+'/'+GetDomainName+'/'+find+".xml", "w",encoding="utf-8")
            myfile.write(mydata)
            print(GetAreaName+' '+GetDomainName+' '+find+' File Converted')  


def converttoxmiPerHelperDiagram():
    Classifier='Helper Diagram'
    if not os.path.exists('XMI BIAN files/'+Classifier):
        folder = os.path.join(os.getcwd(), 'XMI BIAN files/'+Classifier)
        os.makedirs(folder)
    ...
    for i in range(2,len(DName)):
        if Classifier in DName[i]:
            SaveClassifierName.append(DName[i])
    
    while SaveClassifierName:
        find=SaveClassifierName[0]
        SaveClassifierName.pop(0)
        uml = ET.Element('uml:Model')
        uml.set('xmi:version','2.1')
        uml.set('xmlns:xmi','http://schema.omg.org/spec/XMI/2.1')
        uml.set('xmlns:uml','http://www.eclipse.org/uml2/3.0.0/UML')
        ...
        mydata=BeautifulSoup(x,'xml').prettify()
        find=find.replace('/','_')
        myfile = open('XMI BIAN files/'+Classifier+'/'+find+".xml", "w",encoding="utf-8")
        myfile.write(mydata)
        print(find+' File Converted')  

def converttoxmiPerOverviewDiagram():
    Classifier='Overview Diagram'
    if not os.path.exists('XMI BIAN files/'+Classifier):
        folder = os.path.join(os.getcwd(), 'XMI BIAN files/'+Classifier)
        os.makedirs(folder)
